      ...
      <div className='space-y-4'>
        ...
        <div>
          ...
          <div className='mb-2'>
            ...
            <input type='text' value={newSegmentName} onChange={(e) => setNewSegmentName(e.target.value)} className='w-full border px-2 py-1' />
          </div>
          <div className='mb-2'>
            <label className='block mb-1'>Criteria</label>
            <input type='text' value={criteria} onChange={(e) => setCriteria(e.target.value)} className='w-full border px-2 py-1' placeholder='e.g., status: open, company size: >50' />
          </div>
          <button onClick={handleCreateSegment} className='bg-green-500 text-white px-4 py-2 rounded'>Create Segment</button>
        </div>
      </div>
    </Layout>
  );
